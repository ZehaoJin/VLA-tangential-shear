...
def loadcatalog(cluster):
    ##load catalog
    catalog=np.loadtxt('VLA-HFF_%s_compact_optical_rasort.txt' %cluster,dtype={'names': ('name', 'RA', 'DEC','IM_MAJ','IM_MIN','IM_PA','DC_MAJ','DC_MIN','DC_PA','SUBARU_ID','SUBARU_RA','SUBARU_DEC','ZBPZ','ZBPZ_LOWER'),'formats': ('|S26',np.float,np.float,np.float,np.float,np.float,np.float,np.float,np.float,'|S26','|S26','|S26','|S26','|S26')},usecols=(0,1,2,21,23,25,27,29,31,50,51,52,56,57))    
...
def z_cut(cluster,catalog,z,behind1_infrontof2_within3):
    ...
    for i in range(entries):
        ...
        #print catalog[i][13],subaru_zmin[i]
...
def averaged_tangential_shear(cluster,data_DC,data_IM,seperate_DC_IM,ring,shuffle_time,show_scatter,R_cut):
    '''
    ...
    for ring_quantity in ring:
        ...
        all_three_DC_shear=np.array([0.00441468,-0.00638506,-0.03265289,-0.02907252])
        all_three_DC_sigma=np.array([0.01890369,0.01908342,0.01911915,0.01930759])
        all_three_xticks=np.array([1.700,4.473,6.553,8.632])
        ...
        if seperate_DC_IM==False:
             ...
             plt.figure('J'+cluster+'averaged tangential shear for '+str(ring_quantity)+' rings'+'R_cut='+str(R_cut))
             plt.xlabel('$r$  (arcmin)')
             ...
             plt.scatter(xticks,result3_DC[count][:ring_quantity],s=100, color="r",marker='x',label="Radio cross shear of J"+cluster)
             plt.errorbar(all_three_xticks,all_three_DC_shear,all_three_DC_sigma,ecolor=error_bar_color_IM,fmt=data_fmt_IM,label="3 cluster total tangential shear")
             #plt.scatter(xticks,result3_IM[count][:ring_quantity],s=100, color="b",marker='x',label="IM cross shear")
             plt.errorbar(xticks,result_DC[count][:ring_quantity],error_stdev_DC[count][:ring_quantity],ecolor=error_bar_color_DC,fmt=data_fmt_DC,label="Radio tangential shear of J"+cluster)
             ...
             plt.xticks(xticks)
             plt.title('J'+cluster+'   $\overline{\gamma}$   for'+str(ring_quantity)+' rings'+'at R_cut='+str(R_cut))
