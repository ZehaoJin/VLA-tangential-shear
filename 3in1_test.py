...
for cluster in cluster_list:
    ...
    godeye=np.zeros((entries_R,15))
    #godeye[i,0-IM_a,1-IM_b,2-DC_a,3-DC_b,4-IM-PA,5-DC_PA,6-Zbest,7-Zmin,8-IM_tangential_shear,9-DC_tangential_shear,10-IM_cross_shear,11-DC_cross_shear,12-phi]
    ...
    ###tangential_shear[(0-x,1-y,2-tangential_shear,3-cross_shear,4-dc_a,5-dc_b,6-dc_PA,7~end-shuffled_tangential_shear),i]
    ###grid[(phi,x,y,dc_a,dc_b),i]
    for i in range(entries_R):
        godeye[i][0]=catalog[i][3]
        godeye[i][1]=catalog[i][4]
        godeye[i][2]=catalog[i][6]
        godeye[i][3]=catalog[i][7]
        godeye[i][4]=catalog[i][5]
        godeye[i][5]=catalog[i][8]
        '''
        if catalog[i][11]=='-':
            godeye[i][6]=0
        else:
            godeye[i][6]=catalog[i][11]
        if catalog[i][12]=='-':
            godeye[i][7]=0
        else:
            godeye[i][7]=catalog[i][12]
        '''
        godeye[i][8]=tangential_shear_IM[2][i]
        godeye[i][9]=tangential_shear_DC[2][i]
        godeye[i][10]=tangential_shear_IM[3][i]
        godeye[i][11]=tangential_shear_DC[3][i]
        #godeye[i][12]=grid[0][i]
        godeye[i][12]=catalog[i][4]/catalog[i][3]
        godeye[i][13]=catalog[i][7]/catalog[i][6]
        godeye[i][14]=np.sqrt(grid[1][i]**2+grid[2][i]**2)*60
        
        
        
    #print '0-IM_a,1-IM_b,2-DC_a,3-DC_b,4-IM-PA,5-DC_PA,6-Zbest,7-Zmin,8-IM_tangential_shear,9-DC_tangential_shear,10-IM_cross_shear,11-DC_cross_shear,12-phi'
    #print godeye
    
    bins=100
    '''
    plt.figure('clustercentric_R '+'J'+cluster)
    plt.title('clustercentric_R '+'J'+cluster)
    plt.xlabel('R')
    plt.ylabel('quantity')
    plt.hist(godeye[:,14],bins)
    
    plt.figure('b/a (DC) J'+cluster)
    plt.title('b/a (DC) J'+cluster)
    plt.xlabel('b/a')
    plt.ylabel('quantity')
    plt.hist(godeye[:,13],bins)
    
    plt.figure('b/a vs. tangential_shear(DC)'+'J'+cluster)
    plt.title('b/a vs. tangential_shear(DC)'+'J'+cluster)
    plt.xlabel('b/a')
    plt.ylabel('tangential shear')
    plt.scatter(godeye[:,13],godeye[:,9])

    
    plt.figure('clustercentric_R vs. tangential_shear'+'J'+cluster)
    plt.title('clustercentric_R vs. tangential_shear'+'J'+cluster)
    plt.scatter(godeye[:,14],godeye[:,9])
    
    plt.figure('b/a (IM) J'+cluster)
    plt.title('b/a (IM) J'+cluster)
    plt.xlabel('b/a')
    plt.ylabel('quantity')
    plt.hist(godeye[:,12],100)
    '''

    print 'mean',np.mean(godeye[:,13])
    print 'median',np.median(godeye[:,13])
    print godeye[:,13].size
