...
catalog=np.loadtxt('VLA-HFF_%s_compact_optical_rasort.txt' %cluster,dtype={'names': ('name', 'RA', 'DEC','DC_MAJ','DC_MIN','M_STAR','DC_PA','SIGMA_DC_MAJ','SIGMA_DC_MIN','ZBEST'),'formats': ('|S26',np.float,np.float,np.float,np.float,'|S26',np.float,np.float,np.float,'|S26')},usecols=(0,1,2,27,29,43,31,28,30,40))
ra,dec=np.loadtxt('VLA-HFF_%s_compact_optical_rasort.txt' %cluster,usecols=(1,2),unpack=True)
###ra,dec are for making plots
fullentries=catalog.size
if show==True:
    for i in range(fullentries):
        print catalog[i][3],catalog[i][7],catalog[i][4],catalog[i][8],catalog[i][9],catalog[i][7]/catalog[i][3],catalog[i][8]/catalog[i][4]
    print 'DC_MAJ,SIGMA_DC_MAJ,DC_MIN,SIGMA_DC_MIN,ZBEST,SIGMA/MAJ,SIGMA/MIN'
...
print 'Mean complex shear: ',real_noise,'+',imag_noise,'i'
